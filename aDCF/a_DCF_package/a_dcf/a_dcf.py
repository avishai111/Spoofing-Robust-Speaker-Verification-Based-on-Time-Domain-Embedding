...
def calculate_a_dcf_python_old_version(
    asv_scores: np.ndarray,
    asv_labels: np.ndarray,
    cost_model: CostModel = CostModel(),
    printres: bool = True,
    ):

    scores = asv_scores.astype(np.float64)
    keys = asv_labels
    ...
    min_a_dcf = a_dcfs[min_a_dcf_idx]
    min_a_dcf_thresh = a_dcf_thresh[min_a_dcf_idx]
    min_a_dcfs_normed = a_dcfs_normed #this change in the new version! (this for supporting the old version)
    ...
    return min_a_dcf,min_a_dcf_thresh,min_a_dcfs_normed,np.array(a_dcf_thresh)
    

def calculate_a_dcf_python(
    asv_scores: np.ndarray,
    asv_labels: np.ndarray,
    cost_model: CostModel = CostModel(),
    printres: bool = True,
    labels_are_int: bool = False,
    ):
    ...
    # Extract target, nontarget, and spoof scores from the ASV scores
    if labels_are_int == False:
        trg = scores[keys == 'target'] 
        nontrg = scores[keys == 'nontarget']
        spf = scores[keys == 'spoof']
    else:
        trg = scores[keys == 1] 
        nontrg = scores[keys == 0]
        spf = scores[keys == 2]
    ...
    min_a_dcf = a_dcfs[min_a_dcf_idx]
    min_a_dcf_thresh = a_dcf_thresh[min_a_dcf_idx]
    min_a_dcfs_normed = a_dcfs_normed[min_a_dcf_idx]
    x_axis = np.arange(len(a_dcfs_normed))
...
def compute_a_det_curve(trg_scores: np.ndarray, nontrg_scores: np.ndarray, spf_scores: np.ndarray) -> List[List]:
    ...
    for sco, lab in zip(scores_sorted, labels):
        ...
        #Avishai Weizman adding this:
        # if len(nontrg_scores) == 0: # to handle the case where there is no spoof score
        #     nontrg_scores = np.array([0])
            
        # if len(trg_scores) == 0: # to handle the case where there is no spoof score
        #     trg_scores = np.array([1])
        
        # if len(spf_scores) == 0: # to handle the case where there is no spoof score
        #         spf_scores = np.array([2])
