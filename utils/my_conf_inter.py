...
def get_conf_int(values, alpha=5):
    """Compute confidence intervals for scalar or vector values from bootstrapped metrics."""
    if values.ndim == 1:  # Scalar case
        low = np.percentile(values, alpha / 2)
        high = np.percentile(values, 100 - alpha / 2)
        return (low, high)
    else:  # Vector case
        lows = np.percentile(values, alpha / 2, axis=0)
        highs = np.percentile(values, 100 - alpha / 2, axis=0)
        return list(zip(lows, highs))  # Return a list of tuples for each vector element
...
class Bootstrap:
    ...
    def get_metric_values_for_bootstrap_sets(self, samples, labels, samples2=None):
        ...
        # Store results in a list for dynamic shape handling
        vals = []

        for indices in self._indices:
            result = metric_wrapper(self.labels, self.samples, self.samples2, self.metric, indices)
            vals.append(np.array(result) if not np.isscalar(result) else result)

        # Convert to a 2D numpy array for vector support
        self._scores = np.vstack(vals) if isinstance(vals[0], np.ndarray) else np.array(vals)
        return self._scores
